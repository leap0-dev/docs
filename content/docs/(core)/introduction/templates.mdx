...
| `system/code-interpreter:v0.1.0` | Python 3.12 and Bun runtimes with 50+ pre-installed data-science packages. Used with the <a href={`${docsBaseUrl}/code-interpreter`}>Code Interpreter</a> agent tool. |
| `system/desktop:v0.1.0` | XFCE desktop sandbox with noVNC access, screenshots, input automation, window inspection, and screen recording. Used with the <a href={`${docsBaseUrl}/desktop`}>Computer Use (Desktop)</a> agent tool. |
