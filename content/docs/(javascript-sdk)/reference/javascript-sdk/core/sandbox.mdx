...
- The stopped sandbox handle.

#### Throws

- `Leap0Error`: If stopping the sandbox fails.
...
- The started sandbox handle.

#### Throws

- `Leap0Error`: If starting the sandbox fails.

### `createSnapshot`

```ts
async createSnapshot(params: CreateSnapshotParams = {}, options?: { timeout?: number })
```
...
- `params`: Optional snapshot naming and lifecycle parameters.
- `options`: Optional request settings.
...
- `Leap0Error`: If creating the snapshot fails.
